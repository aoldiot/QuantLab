"""1小时布林带均值回归反转策略（优化降频版）。

在原版的基础上，多头与空头入场信号额外增加"收盘价确认"过滤：
- 多头信号：当前1小时K线最低价触及或跌破布林带下轨 **且** 收盘价确认跌破下轨，
  且Choppiness指数低于阈值（按规格归一化到 0~1 后与 chop_threshold 比较，
  0.4 即原始 CHOP < 40）。
- 空头信号：当前1小时K线最高价触及或突破布林带上轨 **且** 收盘价确认突破上轨，
  且Choppiness指数低于阈值。

通过收盘价确认过滤盘中假突破，从而降低交易频率，减少无效交易，
提升策略整体稳定性和风险收益比。

仓位与平仓规则与原版一致：触及中轨平仓，固定比例仓位（默认10%，最大20%），
同时仅持一单。
"""
...
class BollingerMeanReversal1hOptimizedConfig(StrategyConfig, frozen=True):
...
class BollingerMeanReversal1hOptimizedStrategy(Strategy):
    """1小时布林带均值回归反转策略（优化降频版）实现。"""

    def __init__(self, config: BollingerMeanReversal1hOptimizedConfig) -> None:
    ...
    def on_bar(self, bar: Bar) -> None:
        ...
        # 仅在空仓 + Choppiness 确认震荡市时考虑入场
            ...
            # 多头信号：最低价触及下轨 且 收盘价确认跌破下轨
            if bar_low <= bb_down and bar_close <= bb_down:
                self._pending_action = "LONG"
            # 空头信号：最高价触及上轨 且 收盘价确认突破上轨
            elif bar_high >= bb_up and bar_close >= bb_up:
...
STRATEGY_MANIFEST = StrategyManifest(
    slug="bollinger_mean_reversal_1h_optimized",
    name="1小时布林带均值回归反转策略（优化降频版）",
    version="1.0.0",
    description=(
        "在原版1小时布林带均值回归策略的基础上，多头与空头入场信号额外增加"
        "收盘价确认过滤：多头需要最低价触及下轨且收盘价确认跌破下轨，"
        "空头需要最高价触及上轨且收盘价确认突破上轨。结合Choppiness指数"
        "震荡过滤，平仓信号仍为布林带中轨，可显著降低交易频率、过滤盘中"
        "假突破。仓位管理使用账户权益固定比例（默认10%，最大20%），同时仅持一单。"
    ),
    category="mean_reversion",
    strategy_path=(
        "app.strategies.bollinger_mean_reversal_1h_optimized"
        ":BollingerMeanReversal1hOptimizedStrategy"
    ),
    config_path=(
        "app.strategies.bollinger_mean_reversal_1h_optimized"
        ":BollingerMeanReversal1hOptimizedConfig"
    ),
