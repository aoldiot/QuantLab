...
import {ArrowLeft,Check,Code2,FileCode2,History,Play,RotateCcw,Sparkles,Trash2,X} from 'lucide-react'
...
export default function StrategyDetail(){
  ...
  const[content,setContent]=useState(''),[saved,setSaved]=useState(''),[tab,setTab]=useState<Tab>('code'),[busy,setBusy]=useState(false),[publishOpen,setPublishOpen]=useState(false),[aiOpen,setAiOpen]=useState(false),[error,setError]=useState(''),[toast,setToast]=useState('')
  ...
  return <><div className="strategy-title"><Link className="detail-back" to="/strategies" aria-label="返回策略列表" title="返回策略列表"><ArrowLeft/></Link><h1>{strategy?.name??file?.name??name}</h1></div>
    ...
    {tab==='code'&&<div className={'detail-code-layout '+(aiOpen?'with-ai':'')}><div className="detail-editor"><div className="editor-tabs"><span><FileCode2/>{file?.filename}{dirty&&<i>●</i>}</span><div className="editor-actions"><button className={'button ai-toggle '+(aiOpen?'active':'')} onClick={()=>setAiOpen(!aiOpen)}><Sparkles/>Agent 编写</button><button className="button primary" title={!codeChanged?'代码未发生改变，无需重复发布':dirty?'代码正在自动保存':''} disabled={!file||busy||dirty||!codeChanged} onClick={()=>setPublishOpen(true)}><Play/>{strategy?'发布新版本':'首次发布'}</button></div></div><CodeEditor value={content} onChange={setContent}/><div className="editor-status"><span>Python</span><span>{content.split('\n').length} 行</span><span>{dirty?'自动保存中…':codeChanged?'有待发布修改':'已是最新版本'}</span></div></div>{aiOpen&&<AgentPanel strategyName={name} onClose={()=>setAiOpen(false)} onApplied={async()=>{await load();setToast('Agent 修改已应用，请检查并发布新版本')}}/>}</div>}
