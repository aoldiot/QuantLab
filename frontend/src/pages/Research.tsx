...
export default function Research(){
  ...
  async function send(e:React.FormEvent){e.preventDefault();const text=input.trim();if(!project||!text||busy)return;setMessages(old=>[...old,{id:crypto.randomUUID(),role:'user',content:text,message_type:'message',metadata:{},created_at:new Date().toISOString()}]);setInput('');setBusy(true);setError('');try{await api.sendResearchMessage(project.id,text);await open(project);await reloadList()}catch(e){setError((e as Error).message)}finally{setBusy(false)}}
  ...
  async function startBacktest(e:React.FormEvent<HTMLFormElement>){e.preventDefault();if(!project||!preview)return;const f=new FormData(e.currentTarget),params:Record<string,unknown>={};Object.entries(preview.parameter_schema).forEach(([key,spec])=>{const raw=f.get('param_'+key);params[key]=spec.type==='boolean'?raw==='on':spec.type==='integer'||spec.type==='number'?Number(raw):raw});setBusy(true);try{const published=await api.publishResearchStrategy(project.id);await api.createResearchRun(project.id,{name:f.get('name'),strategy_version_id:published.latest_version_id,strategy_parameters:params,venue:'BINANCE',symbols:String(f.get('symbols')).split(',').map(x=>x.trim()).filter(Boolean),timeframes:preview.data_requirements.timeframes,start_date:f.get('start'),end_date:f.get('end'),initial_balance:Number(f.get('capital')),leverage:Number(f.get('leverage')),execution_model:f.get('model'),funding:Boolean(preview.data_requirements.funding),catalog_path:f.get('catalog_path')||null});setBacktestOpen(false);await open(project)}catch(e){setError((e as Error).message)}finally{setBusy(false)}}
