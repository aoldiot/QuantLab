...
function collectText(value:unknown,depth=0):string[]{if(depth>10||typeof value==='string')return[];if(Array.isArray(value))return value.flatMap(child=>collectText(child,depth+1));if(value&&typeof value==='object'){const item=value as Record<string,unknown>;const own=typeof item.text==='string'?[item.text]:[];return own.concat(Object.entries(item).filter(([key])=>key!=='text').flatMap(([,child])=>collectText(child,depth+1)))}return[]}
...
export default function AgentPanel({strategyName,onClose,onApplied,initialPrompt,sessionId}:{strategyName:string;onClose:()=>void;onApplied:()=>Promise<void>;initialPrompt?:string;sessionId?:string}){
  ...
  const[sessions,setSessions]=useState<AgentSession[]>([]),[historyCollapsed,setHistoryCollapsed]=useState(false),[sessionPreviews,setSessionPreviews]=useState<Record<string,string>>({}),[thinkingTokens,setThinkingTokens]=useState(0),[retry,setRetry]=useState<{attempt:number;max:number}|null>(null)
  ...
  function connect(created:AgentSession){
    ...
    ws.onmessage=e=>{const data=JSON.parse(e.data);if(data.type==='status'){setStatus(data.status);if(data.status==='RUNNING')initialPromptStarted.current=true;if(data.status==='IDLE'&&initialPromptStarted.current)initialPromptCompleted.current=true}if(data.type==='queued'){initialPromptStarted.current=true;setThinkingTokens(0);setRetry(null);setStatus('QUEUED')}if(data.type==='api_retry')setRetry({attempt:Number(data.attempt??0),max:Number(data.max_retries??0)});if(data.type==='context_usage')setContext({percentage:Number(data.usage.percentage??0),totalTokens:Number(data.usage.totalTokens??0),maxTokens:Number(data.usage.maxTokens??0)});if(data.type==='error'){setRetry(null);setStatus('FAILED');setError(data.message||'Agent 执行失败，请重试或查看后端日志。')}if(data.type==='backtest_error')setError(data.message);if(data.type==='approval_required')setApproval(data);if(data.type==='changes_ready'){initialPromptCompleted.current=true;setChanges(data)}if(data.type==='backtest_created')setLines(old=>[...old,{role:'system',text:`已创建正式回测记录 ${data.name}（${data.run_id}）`}]);if(data.type==='sdk_event'){if(data.event?.subtype==='thinking_tokens')setThinkingTokens(Number(data.event?.data?.estimated_tokens??0));if(data.event?.subtype==='error_during_execution'){const message=Array.isArray(data.event?.errors)&&data.event.errors.length?data.event.errors.join('；'):'Agent 执行被中断或模型调用失败';const isInterrupted=message.includes('interrupted')||message.includes('interruption')||message.includes('被中断');if(!isInterrupted){setError(message);setLines(old=>[...old,{role:'system',text:message}])}}if(data.event?.message_type==='AssistantMessage'){initialPromptCompleted.current=true;setRetry(null);const tools=toolLines(data.event.content);const text=[...new Set(collectText(data.event.content))].join('\n');if(tools.length||text)setLines(old=>[...old,...tools,...(text?[{role:'assistant' as const,text}]:[])])}}}
  }
  ...
  async function openSession(current:AgentSession){closeCurrentSocket();setError('');setRetry(null);setApproval(null);setShowDiff(false);try{const[saved,pending]=await Promise.all([api.agentMessages(current.id),api.agentDiff(current.id)]);const restored=messageLines(saved);setLines(restored);setThinkingTokens(savedThinking(saved));setChanges(pending.files.length?pending:null);setContext(savedContext(saved));setSessionPreviews(old=>({...old,[current.id]:restored.find(line=>line.role==='user'&&!isHandoffPrompt(line.text))?.text??'策略开发'}));connect(current)}catch(e){setError((e as Error).message)}}
  useEffect(()=>{let active=true;mounted.current=true;api.agentSessions(undefined,strategyName).then(async all=>{if(!active)return;setSessions(all);const previews=await Promise.all(all.map(async item=>{const restored=messageLines(await api.agentMessages(item.id));return[item.id,restored.find(line=>line.role==='user'&&!isHandoffPrompt(line.text))?.text??'策略开发'] as const}));if(!active)return;setSessionPreviews(Object.fromEntries(previews));const target=(sessionId&&all.find(item=>item.id===sessionId))||all[0];if(target)await openSession(target)}).catch(e=>setError((e as Error).message));return()=>{active=false;mounted.current=false;closeCurrentSocket()}},[strategyName,sessionId])
  useEffect(()=>{const frame=requestAnimationFrame(()=>{const element=conversationRef.current;if(element)element.scrollTo({top:element.scrollHeight,behavior:'smooth'})});return()=>cancelAnimationFrame(frame)},[lines,changes,status,retry])
  ...
  async function newSession(){if(running)return;closeCurrentSocket();setError('');setRetry(null);setLines([]);setChanges(null);setContext({percentage:0,totalTokens:0,maxTokens:0});try{const created=await api.createAgentSession(clientId(),strategyName,mode);setSessions(old=>[created,...old]);connect(created)}catch(e){setError((e as Error).message)}}
